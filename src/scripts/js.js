let createSvg = function (name) {
    return `<svg class="icon" aria-hidden="true" type="${name}" status="no">
...
class Box {
    ...
    bindDomEvent(dom) {
        ...
        dom.addEventListener('click', function (e) {
            let target = e.target
            if (target.className === 'card' && target.firstElementChild.getAttribute('status') === 'no') {
                that.clickCard(target)
            }
        }, false)
    }
    clickCard(card) {
        //首先判断card是否被遮挡，如果是，则不能被点击
        let x = parseInt(card.getAttribute('row'))
        let y = parseInt(card.getAttribute('col'))
        let z = parseInt(card.getAttribute('deepth'))
        console.log(z, x, y)
        ...
        card.firstElementChild.setAttribute('status', 'yes')
        let that = this
